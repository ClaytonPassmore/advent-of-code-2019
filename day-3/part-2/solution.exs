defmodule Day3.Part1 do
  ...
  def segmentify(instruction, accumulator) do
    ...
    case direction do
      "L" -> [ { x - distance, y, d + distance } | accumulator ]
      "R" -> [ { x + distance, y, d + distance } | accumulator ]
      "D" -> [ { x, y - distance, d + distance } | accumulator ]
      "U" -> [ { x, y + distance, d + distance } | accumulator ]
    end
  end
  ...
  @doc """
    ...
    Return the sum of each wire's step distance from the origin (0, 0) to the POI.
    If there is no POI, return 0.
  """
  def hv_intersect(horizontal, vertical) do
    { { h_x1, h_y, h_d }, { h_x2, _, _ } } = horizontal
    { { v_x, v_y1, v_d }, { _, v_y2, _ } } = vertical
    ...
    if (h_min_x <= v_x && v_x <= h_max_x && v_min_y <= h_y && h_y <= v_max_y) do
      h_d + abs(v_x - h_x1) + v_d + abs(h_y - v_y1)
    else
      # No intersection.
      0
    end
  ...
  @doc """
    Calculate the sum of each wire's step distance to the point of intersection between
    line 1 and line 2. If there is no point of intersection, return 0.
  ...
  @doc """
    Find all the points of intersection between two wires and compute the sum of the step distances
    to those POIs.
  """
  ...
  @doc """
    Turn the wire instructions into line segments, then find the sum of step distances to the
